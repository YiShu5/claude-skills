---
name: 意疏的AI口袋
description: 微信公众号文章的端到端创作发布工具链（公众号品牌「意疏的AI口袋」）。触发场景包括 — "写一篇关于 X 的公众号文章" / "我有稿子要发到公众号" / "帮我做张公众号封面" / "改写这篇 mp.weixin 链接" / "把稿子发到草稿箱" / "这文章 AI 味太重 / 降 AI 味" / "选题不知道写啥" / "把 markdown 转成公众号能用的"。覆盖选题、写作、三遍审校、Markdown→微信 HTML、封面与配图生成、上传素材、创建/发布草稿。不处理朋友圈、视频号、小程序内容。
---
...
1. **风格参考必读**：写作前必须**至少读 1 份范文**，按场景选：
   ...
   - **以上都不像**（生活随笔 / 节日感想 / 新闻快讯 / 热点跟进 / 思辨讨论等） → 默认参考 `references/范文-意疏精选.md` 全集 + 共性 DNA 6 条
   仅靠 `HUMANIZE_GUIDE.md` 的"避免清单"不够 —— **必须有正面范本**。
2. **不编造**：搜不到的数据 / 案例直接删，或写"暂未找到公开数据"。涉及 2024-2025 新产品 / 政策 / 数字 → 必须 web_fetch 核实。
3. **三遍审校**（A/B/C 强制，详见 `references/三遍审校清单.md`）：
   - 一审 内容：事实 / 逻辑 / 结构
   - 二审 风格：删 AI 套话 + 加 ≥10 处个人态度
   - 三审 细节：句长 / 全角标点 / 中英文空格
   未通过审校 → 不出图 → 不建草稿。
4. **签名结尾**：写完正文后**直接调用** `references/意疏-签名结尾.md` 的标准版，不要让模型自由发挥结尾。

### 参考参数（非硬约束，服从内容）

- **字数**：常规 1200-1500，技术评测 1500-2000。不凑数。
- **段落节奏**：刻意短-长-短交替，不要每段都 50-80 字。
- **个人态度**：全文至少 3 处自嘲 / 反讽 / 情绪化口语。

## 发布核心约束

5. **必须把 Markdown 转内联 HTML**：微信编辑器不识别 `<style>`。
   `create_draft.py --content` 只接受 `markdown_to_wechat_doocs.py` 的输出，**绝不直传 Markdown**。
6. **主题选择**：按 `THEME_GUIDE.md` 的内容→主题映射；用户在 `.env` 设了 `MARKDOWN_THEME` 优先用。
7. **图片必须按五要素结构**（详见 `references/image-prompts.md`）：核心意象 / 情绪基调 / 画面风格 / 构图 / 负面提示。封面右上角嵌入「意疏的AI口袋」。配图 3 张按"痛点 → 转折 → 升华"故事弧线，prompt 互不相同。**禁止**「XX 风格插画」这种两要素空泛 prompt。
...
| 文档 | 何时**必须**读 | 何时**跳过** |
| --- | --- | --- |
| `references/范文-意疏精选.md` | A/B/C 任何写作任务前（除 D 审校）| D 任务 |
| `references/范文-卡兹克精选.md` | 写评测 / 行业评论 / 故事 / 调查 / 自有产品 | 纯保姆教程类（用意疏 A 集足够）|
| `references/意疏-签名结尾.md` | 写完正文拼结尾时 | D 审校 / E 问答 |
| `references/三遍审校清单.md` | 三遍审校执行时 | 仅预览 / 仅出图等不写正文的任务 |
| `references/viral_titles_50.md` | 任务 B 阶段 2 起候选标题 | 任务 A（已有标题）/ D / E |
| `references/image-prompts.md` | 写封面或配图 prompt 时 | 仅预览模式（不出图）/ D / E |
| `references/wechat-api.md` | 微信 API 报错或想理解错误码时 | 正常流程跑通时 |
| `references/examples.md` | 不熟悉发布管道、想看完整 bash 例子时 | 已熟悉发布管道时 |
| `THEME_GUIDE.md` | 用户没指定主题、需要按内容选时 | `.env` 已设 `MARKDOWN_THEME` 时 |
| `HUMANIZE_GUIDE.md` (12KB) | 二审"删 AI 套话"卡壳、想找具体替换词时 | **平时跳过**，不要默认加载 |
| `CHICKEN_SOUP_STYLE.md` (11KB) | 写 AI 工具评测 / 编程 / Prompt 工程类 | 其它题材**跳过** |
